...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        const lineItems = products.map((product) => {
            const amount = Math.round(product.price * 100); // Stripes wants amount in cents
        ...
        res.status(200).json({ id: session.id, totalAmount: totalAmount / 100 }); // Convert back to dollars
    } catch (error) {
